...
header-includes:
  ...
  - \pretitle{\begin{center}
    \includegraphics[width=3in,height=3in]{img/lmu.png}\LARGE\\\vspace{100pt}}
  - \posttitle{\vspace{230pt}\end{center}}
output: 
  pdf_document:
...
1. Preparing the data and defining the target variable;
2. Dealing with missing values by using different methods;
3. Transforming nominal variables into numeric variables;
4. Training all datasets with multiple models;
5. Selecting one dataset for fine-tuning;
6. Fine-tuning all models with the selected dataset.
...
According to (Bishop, Christopher (2006)), in machine learning, a feature is an individual measurable property or characteristic of a phenomenon being observed, and the concept of "feature" is related to that of explanatory variables used in statistic application. Therefore, both concepts mentioned in this report are the same based upon the definition of feature and variable. In this study, all features are classified into three kinds of variables according to their data type: binary variables, categorical variables, and continuous variables. The description of all features and a self-defined response variable will be provided in this section.
...
A payment default is in simple terms an overdue account. In short, in most cases it arises when a debt has become overdue, and based on the Basel Agreement (2007) and Guidance to banks on non-performing loans from European Central Bank (ECB), the potential client, whose debt is not paid more than 60 days of the payment date printed on the invoice by the credit provider or the lender has reason to believe that the loan will not be repaid, will be classified as risk users.
...
# for each ID, if the target value > 0, 
...
summary(tempData$imp$OCCUPATION_TYPE)
```
 
In order to get a whole dataset, we can complete the data frame with the imputed ones.

```{r, eval=FALSE, echo=FALSE}
#complete the whole dataset with the imputed variable
...
summary(completeData$OCCUPATION_TYPE)
```
...
3. Calculate the percentage of events and percentage of non-events for each variable value. In the example above, percentage of events of binary variable 1(binary value is 1), represents the proportion of sample that both target(y) value and variable value are 1 to the sample that target value is 1. $$\%of\ events(a\ variable\ type) = \frac{\#sample(target=1,variable=1)}{\#sample(target=1)}$$
and percentage of nonevents represents the proportion of sample that  target(y) value is 0 and variable value are 1 to the sample that target value is 0. $$\%of\ nonevents(a\ variable\ type) = \frac{\#sample(target=0,variable=1)}{\#sample(target=0)}$$.
4. Calculate WOE for a variable type by taking natural log of division of percentage  of non-events and percentage of events.  
...
# calc_iv function compute the "information value" of each category of variables.
calc_iv <- function(feature) {
  ...
  for (i in seq(number_row)) {
    ...
    # compute and store the number of positive and negative sample within this level
  ...
  inner_iv_table <- inner_iv_table %>%
    ...
    # compute bad rate, distributions and woe
  ...
  # compute IV and store iv in iv_table 
...
convert_binary_variable <- function(feature) {
  ...
  # select binary banary variables.
...
dl_bin <- woebin(converted_data,
  y = "y",
  x = c("CNT_CHILDREN", "AMT_INCOME_TOTAL", "DAYS_BIRTH",
...
+-----------------+---------------+---------------+-----------------+---------------+---------------+-----------------+
|                 |     dl_iv     |    mf_iv      |     mice_iv     |    dl_oh      |    mf_oh      |     mice_oh     |
+=================+===============+===============+=================+===============+===============+=================+
|     Logistic    |  **0.7248**   |    0.7201     |     0.7170      |    0.6755     |    0.6688     |     0.6650      |
+-----------------+---------------+---------------+-----------------+---------------+---------------+-----------------+
|       KNN       |    0.7143     |    0.6988     |     0.6944      |  **0.7216**   |    0.6813     |     0.6776      |
+-----------------+---------------+---------------+-----------------+---------------+---------------+-----------------+
|       SVM       |  **0.5218**   |    0.5170     |     0.4770      |    0.4851     |    0.4920     |     0.5079      |
+-----------------+---------------+---------------+-----------------+---------------+---------------+-----------------+
|        RF       |  **0.7643**   |    0.7581     |     0.7569      |    0.7433     |    0.7280     |     0.7239      |
+-----------------+---------------+---------------+-----------------+---------------+---------------+-----------------+

To reduce our computation cost, we decided to take the task, which generally has the best prediction performance among all the six tasks with regards to 4 different models, being **dl_iv_data**. Now, we will focus on the task **dl_iv_data**, and fine-tune the parameters.
...
Since there is no parameter to tune in Logistic Regression, we will skip this section.
...
knn_smote_param_set <- ParamSet$new(params = 
                        list(ParamInt$new("smote.dup_size", lower = 20, upper = 60),
                            ParamInt$new("smote.K", lower = 10, upper = 20)))
...
para_dist <- ParamSet$new(params = list(ParamInt$new("classif.kknn.distance", 
                                                     lower = 1, upper = 5)))
...
ggarrange(over_dist_gg1, over_dist_gg2, over_dist_gg3, 
...
bmr_over_k$aggregate(measures = msr("classif.auc"))[[7]]
...
Finally, we want to tune *k* with kernels we selected in the previous section. Beside *epanechnikov* being lowest in all plot, there is no straightforward winner between *triangular* and *biweight*. We can see that AUC is the highest in the range of 40 to 80. If we take a closer look in this range, we notice that *triangular* is slightly better. Based on the trend of the 3 plots above, it is reasonable to select *triangular* as our kernel, with *k* set to 80. Finally, after balancing data with **Oversampling** we get a model with AUC 0.7594.
...
ggarrange(smote_dist_gg1, smote_dist_gg2, smote_dist_gg3, 
          common.legend = TRUE, legend="bottom")
...
ggarrange(smote_kern_gg1, smote_kern_gg2, smote_kern_gg3, 
          common.legend = TRUE, legend="bottom")
...
ggarrange(smote_k_gg1, smote_k_gg2, smote_k_gg3, 
          common.legend = TRUE, legend="bottom")


bmr_smote_k$aggregate(measures = msr("classif.auc"))[[7]]
...
Furthermore, the plot above shows as *k* increase the difference between kernels becomes larger. This suggests that it would be meaningful to tune all *kernel* among *k*. However, since there is no enormous improvement in AUC and the limitation of computation power, we only tuned 3 in our experiment. Finally, after balancing data with **SMOTE** we get a model with AUC 0.7730.
...
param_kernel <- ParamSet$new(params = list(ParamFct$new("classif.svm.kernel", 
                                                        levels = kernel_type)))
...
param_cost <- ParamSet$new(params = list(ParamDbl$new("classif.svm.cost",
                                                      lower = 0, upper = 3)))
...
As mentioned in chapter 3, we fix the parameter *num.trees* with value 1000 and set training range 30 to 60 for *smote.dup_size* and 10 to 20 for *smote.K* seperately in order to find the best training pairs with the two parameters, reaching the best prediction performance.
...
rf_over_param_set <- ParamSet$new(list(ParamDbl$new("oversample.ratio",
                                                    lower = 30, upper = 60)))
...
## Conclusion
In general, we have implemented four different training models (**Logistic Regression**, **KNN**, **SVM** and **Random Forest**) on the same training task (**dl_iv_data**) to decide whether or not to approve the issuance of the credit card to potential customers. Besides the basic training process with default parameter settings with the above four models, we have also discovered the necessary methods (the **SMOTE Operator** and the **Oversampling Operator**) to tackle with the imbalanced target variable _y_ and tune the models with the corresponding parameters belonging to each model afterwards. By plotting the changing trend of specified parameters with the corresponding prediction performances measured by the **AUC** score separately, we shall be able to determine for the best parameter setting combinations and achieve the highest AUC score with the ultimate chosen model. 

The ultimate prediction performances are shown in the following table including training results with default parameter settings, model tuning prediction results after the **Oversampling Operator** and the **SMOTE algorithm**.


```{r, echo=FALSE}
library(kableExtra) # load required package for the data table with performance output shown
library(tidyverse)

# load the required columns and rows
dt <- tibble(
   Items = linebreak(c("Default AUC", "Over.AUC", "Over.ratio", "Smote.AUC", "smote.k\nsmote.dupsize", "Model Para.")),
   Text_1 = linebreak(c("0.7248", "0.7236", "30", "0.7253", "15\n31", "None")),
   Text_2 = linebreak(c("0.7143", "0.7594", "25", "0.7730", "15\n40", "k=40\ndistance=3\nkernel=triangular")),
   Text_3 = linebreak(c("0.5218", "N/A", "N/A", "0.7839", "21\n40", "cost=0.1\ngamma=0.75\nkernel=radial")),
   Text_4 = linebreak(c("0.7643", "N/A", "N/A", "0.8321", "15\n45", "mtry=7\nnum.trees=1000\nmin.node.size=3\nsample.fraction=0.575"))
)
# show the table
dt %>%
   mutate_all(linebreak) %>%
   kable("latex", booktabs = T, escape = F,
      col.names = linebreak(c("Models", "Logistic Regression", "KNN", "SVM", "Random Forest"), align = "c"))
```

After determining for the related parameters which are used for data balancing and finishing the model tuning procedures with parameters accordingly, we have run into the  conclusion that the **Random Forest** algorithm has shown a considerably better prediction performance measured by the **AUC** value, with an average of 10% improvement among all other training models, in regards to the decision on whether or not to issue the credit card to the potential customer. As for our purpose to predict the ultimate decision concerning the credit card approval, we would choose the **Random Forest** algorithm and implement the classification afterwards with the data collected among various customers to predict for the approval of the credit card.

## Discussion

Based on our previous work, there are still many aspects can be improved in our article. First of all, more powerful models that we can use to solve some problems that we met. For example, it's computationally expensive to train the SVM model, a 3*3 nested resampling of SVM model takes 5 hours. Secondly, we have tried xgboost and we found it extremly fast to excute but with the worst prediction performance of all (AUC values are less than 0.5 and did not improve after any kinds of model tuning).
On the other hand, there are still a lot of cutting-edge models we haven't used, e.g. lightGBM(not included in mlr3 yet), Deep learning method(require high computational power). Last but not least, there are still areas to explore in our feature engineering procedure. For example, dimension reduction methods such as PCA, MDS and Factor analysis (included in mlr3 filters) can also be used.
...
[2] Basel Agreement(2007) Basel III: international regulatory framework for banks.  

[3] Guidance to banks on non-performing loans.

[4] He, Y., Zaslavsky, A. M., Landrum, M. B., Harrington, D. P., & Catalano, P. (2010). Multiple imputation in a large-scale complex survey: a practical guide. Statistical methods in medical research, 19(6), 653-670.

[5] Stuart, E. A., Azur, M., Frangakis, C., & Leaf, P. (2009). Multiple imputation with large data sets: a case study of the Children's Mental Health Initiative. American journal of epidemiology, 169(9), 1133-1139.
...
[7] Jeni, L. A., Cohn, J. F., & De La Torre, F. (2013, September). Facing imbalanced data--recommendations for the use of performance metrics. In 2013 Humaine association conference on affective computing and intelligent interaction (pp. 245-251). IEEE.

[8] Probst, P., Wright, M. N., & Boulesteix, A. L. (2019). Hyperparameters and tuning strategies for random forest. Wiley Interdisciplinary Reviews: Data Mining and Knowledge Discovery, 9(3), e1301.

[9] SVM Hyperparameter Tuning using GridSearchCV. https://towardsdatascience.com/svm-hyper-parameter-tuning-using-gridsearchcv-49c0bc55ce29 Jan 2018.

[10] Understanding Random Forests Classifiers in Python. https://www.datacamp.com/community/tutorials/random-forests-classifier-python%23algorithm May 2018.
